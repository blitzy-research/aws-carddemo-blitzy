...
-- rows and sign-on identities are V3 and V4, numbered ABOVE that ceiling, so only the local and test
-- profiles - which lift the ceiling to latest - execute them. There is no schema/ or seed/ child
-- directory under db/migration and none may be created: a location is scanned recursively, so a child
-- directory would be reached by any profile that resolves the parent and could separate nothing.
--
