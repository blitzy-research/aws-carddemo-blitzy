...
-- nothing else: exactly three nonunique B-tree secondary indexes standing in for the three legacy
...
-- applies versions in ascending order across ALL configured locations - 1, then 2, then the seeds
-- where a profile resolves them - so any later seed lands against a schema whose foreign keys are
-- already in force and is constraint-checked as it is written. The BATCH_-prefixed job-repository
-- tables are provisioned by Spring Batch itself rather than by any migration, and are independent of
-- this file in both directions: it names none of them and they name no application table.
--
...
--
--  1. EXACTLY NINE EXECUTABLE STATEMENTS: three index creations, then six constraint additions. The
--     counts are contractual and are asserted by schema tests that read PostgreSQL's own catalogs.
--
...
--     is touched. V2 only adds. NO OTHER PERSISTENT OBJECT IS CREATED: no table, sequence, view,
--     routine, trigger, extension, schema or row of data, which is why this migration is safe to
--     apply in production unchanged.
--
...
--
-- Zero tables, sequences, views, routines, triggers, extensions, schemas or rows were created. Zero
-- foreign keys touch daily_transaction, zero use account.acct_group_id, and zero target a
