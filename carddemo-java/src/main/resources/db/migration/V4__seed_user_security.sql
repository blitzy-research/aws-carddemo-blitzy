...
--
-- THIS FILE IS NUMBERED ABOVE THE PRODUCTION CEILING. All four scripts sit FLAT in
-- classpath:db/migration, the one location every profile resolves, and there is no schema/ or seed/
-- child directory - none exists and none may be created, because a Flyway location is scanned
-- recursively and a child directory is therefore reached by any profile that resolves the parent. Do not
-- create one, and do not move this file out of db/migration.
--
...
-- docs/decision-log.md DL-127, which supersedes DL-102 and DL-119.
--
--   CONTROL 1 - THE CEILING. The four scripts are physically flat in one directory, so a
--   directory-scoped location list can express no separation; the script VERSION can, and it is part of
--   this file's own name:
--
...
--            V3__seed_reference_data.sql        \  above the production ceiling, so reported above
--            V4__seed_user_security.sql (this)   >  target and never applied there
--
...
--
--   WHY THE CEILING AND NOT A DIRECTORY. An intervening revision introduced schema/ and seed/ child
--   directories to express this by location. That contradicted the delivered flat layout and left two
--   competing migration models in one module, and it could not work in principle either: a Flyway
--   location is scanned RECURSIVELY, so a child directory is reached by any profile that resolves the
--   parent, and a location list that is identical in every profile draws no boundary. The version is the
--   property that can carry it, because it is part of this file's own name and cannot drift from the
--   file. DL-127 records the reinstatement of the flat layout and supersedes DL-119.
--
--   THE RULE FOR A NEW SEED: put it in this directory and number it above 2. Do not create a child
--   directory. Raise the ceiling only in the same commit that adds a production-required script above
--   it, and never to admit a seed.
--
...
--   set. The same class refuses any resolved location other than the one packaged
--   classpath:db/migration, because another location could present its own V1 and V2 carrying content
--   the ceiling never measured.
