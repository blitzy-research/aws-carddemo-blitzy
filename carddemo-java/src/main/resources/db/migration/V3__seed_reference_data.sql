...
-- nine tables, and nothing else. This file sits FLAT in db/migration beside the schema scripts, the one
-- location every profile resolves, and what holds it out of production is its VERSION: production pins
-- spring.flyway.target=2, so this script and V4 are discovered, reported as above-target, and never
-- executed there, while the local and test overlays lift the ceiling to latest and apply them. There is
-- no schema/ or seed/ child directory under db/migration and none may be created - a location is scanned
-- recursively, so a child directory would be reached by any profile that resolves the parent and could
-- separate nothing. See PROFILE APPLICABILITY below:
--
...
--
-- The boundary is drawn by LOCATION, and four independent controls hold it:
--
--   The separation is by VERSION, because the four scripts are physically flat in one directory that
--   every profile resolves and a directory-scoped location list can therefore express no separation at
--   all:
--
--       classpath:db/migration  -> resolved by EVERY profile
--           V1__create_schema.sql             \
--           V2__create_indexes.sql             >  APPLIED everywhere
--       ------------------------------------------- ceiling: spring.flyway.target: 2
--                                                   (application.yml, application-prod.yml)
--           V3__seed_reference_data.sql       \  above the production ceiling, therefore reported
--           V4__seed_user_security.sql         >  above target and never applied there
--
--   There is no schema/ or seed/ child directory and none may be created. A location is scanned
--   recursively, so a child directory is reached by any profile that resolves the parent; the script
--   NUMBER is part of the file name and cannot drift from the file, which is what makes the ceiling a
--   real boundary.
--
...
--
--   WHY THE CEILING AND NOT A DIRECTORY. An earlier revision introduced schema/ and seed/ child
--   directories to try to express this by location, which contradicted the delivered flat layout and
--   left two competing migration models in one module. A location cannot separate what a recursive scan
--   reaches anyway, and the ceiling can: it is measured against the version, which is part of this
--   file's own name. Code enforcement stands behind the configured value rather than beside a second
--   mechanism. See docs/decision-log.md DL-127.
--
--   WHAT MUST NOT CHANGE. Do not create a child directory under db/migration, do not move this file out
--   of db/migration, do not raise or remove target on the base or production profile, and do not
--   renumber this file to a version at or below 2 - checksum validation is on, so that attempt stops a
--   deployment rather than passing quietly. Any further seed script belongs in this directory and must
--   carry a version above 2.
--
...
--   location other than the one packaged classpath:db/migration, because another location could present
--   its own V1 and V2 carrying content the ceiling never measured. The controls fail in the same
--   direction and none is relied on alone: the setting is visible in the profile documents and invisible
--   in code, and the refusals are unconditional in code and invisible in the documents.
--
...
--   storing the padding as well would double it. Trailing blanks are preserved in exactly four
--   places, each of which is behaviourally significant and documented at its table below:
--   account.acct_group_id, disclosure_group.dis_acct_group_id, daily_transaction.dalytran_proc_ts
--   and daily_transaction.dalytran_source.
--
--   The rule that decides which of those two treatments a column gets is the column's KIND, never its
--   SQL type, and it is stated here once because getting it wrong is invisible until something tries
--   to interpret the value:
--
--       display text  - free-form prose written for a human to read: a name, an address line, a
--                       description, a merchant name or city. Nothing in the module compares it to a
--                       fixed vocabulary, so the padding carries no information and is re-applied by
--                       the fixed-width writer on output. Right-trimmed here.
--       coded field   - a value drawn from a closed vocabulary, or one a key or a lookup is built
--                       from. The padding is part of the value, because the vocabulary is declared at
--                       the legacy field's width and an equality test against a trimmed value would
--                       match nothing. Preserved here, at the exact layout width.
--
--   Applying that rule to every coded column this file writes: dalytran_source is ten characters
--   because DALYTRAN-SOURCE is PIC X(10) and com.carddemo.domain.enums.TransactionSourceType declares
--   its three values at that width, so it is seeded padded; dis_acct_group_id is ten characters for
--   the same reason and is the key of the interest-rate lookup; acct_group_id is ten blanks, which is
--   itself the coded value the fixture holds; dalytran_proc_ts is twenty-six blanks, which is how the
--   record says "not yet processed". The remaining coded columns are single characters and so cannot
--   be trimmed at all - acct_active_status, card_active_status and pri_card_holder_ind here, plus
--   sec_usr_type in V4. tran_source is the posted twin of dalytran_source and is equally a coded
--   field, but this file seeds no transaction row, so the posting job is what writes it, moving the
--   ten characters across at the same width.
--
-- -------------------------------------------------------------------------------------------------
-- ZONED DECIMAL
-- -------------------------------------------------------------------------------------------------
...
--   store into a two-decimal field truncates. A negative-zero image decodes to 0.00, which is the
--   only zero a numeric column holds. The same tables and the same truncating scale live in
--   com.carddemo.util.ZonedDecimalCodec, which is what reads these fields at run time; the two must
--   agree byte for byte.
--
-- -------------------------------------------------------------------------------------------------
-- ANOMALIES CARRIED FAITHFULLY, AND DECISIONS TAKEN - see docs/decision-log.md
-- -------------------------------------------------------------------------------------------------
--   1. account.acct_addr_zip holds 'A000000000' and account.acct_group_id holds ten blanks in all
...
--      of app/data/ASCII/custdata.txt, under the one non-production fixture key that
--      application-local.yml and both copies of application-test.yml declare as a bare literal. Each
--      is 101 characters: the ENC1: marker, then Base64 of a 96-bit initialisation vector, the
--      ciphertext and the 128-bit authentication tag. The identifier is invented and the key seals
--      nothing outside a database rebuilt from these migrations, which is what makes a sealed literal
--      safe to commit; no key material is added by this file.
--
--      EACH ENVELOPE IS BOUND TO THE COLUMN IT IS STORED IN, and that is not decoration. The sealed
--      payload is the column's own binding name, the ASCII unit separator, and then the twenty
--      characters - which is why an envelope measures 101 characters here rather than the 69 an
--      unbound twenty-character payload would produce. Every reader of this column in the module
--      opens it through the field-bound reveal, which refuses an envelope written for any other
--      column, so an unbound literal would authenticate under the key and then be refused by every
--      one of those readers: the account view transaction, the account update transaction and the
--      statement job would each fail on every seeded row. A seed whose values its own application
--      cannot read is not a seed, so the binding is part of the literal rather than something a
--      later pass adds. Reasoning in docs/decision-log.md DL-103.
--
--      AES-GCM draws a fresh initialisation vector per call, so these envelopes cannot be regenerated
--      identically and are fixed literals. A literal cannot be re-keyed, so any edit here must go back
--      through the application service rather than being hand-written. Two components hold that:
--      com.carddemo.config.SeededIdentifierSealingCallback - local and test profiles only - opens every
--      stored value on the after-migrate event and fails start-up on the first that will not open,
--      which is the only check that distinguishes a readable envelope from a wrongly-keyed one, since
--      both carry the ENC1 marker; and SeededProtectedIdentifierIT opens all fifty through the
--      application service and compares each recovered value against the fixture record it came from.
--      The after-migrate event is raised after this migration has committed, so that work is a separate
--      transaction; a failure aborts start-up, and both halves of the callback are idempotent, so the
--      corrected start-up simply runs them again. Production applies V1 and V2 only and does not carry
--      the callback. Reasoning in docs/decision-log.md DL-103.
...
--      ten-character key field, which pads it. Trimming either key, or shortening the first to a bare
--      'A', breaks the lookup outright. The zero rates of the ZEROAPR group are genuinely seeded, but
--      they do NOT make the accrual zero-rate branch reachable from this seed: as anomaly 1 explains,
--      every account misses its first probe and re-probes as the fallback group, whose rate on the
--      (01, 0001) type and category every seeded balance uses is 15.00, so a seed-only run always
--      computes. Covering the skip needs an account constructed with 'ZEROAPR   ' plus a matching
--      category balance.
...
--
-- dalytran_source is seeded at its full ten characters, trailing blanks included, because it is a
-- coded field and not display text - see the trim rule in the header. DALYTRAN-SOURCE is PIC X(10),
-- the column is VARCHAR(10), com.carddemo.util.DailyTransactionRecordMapper reads bytes [22:32)
-- untrimmed, and com.carddemo.domain.enums.TransactionSourceType declares its three values at that
-- same width - 'POS TERM  ', 'OPERATOR  ' and 'System    '. Storing the eight-character spelling
-- would put a value in the column that the enumeration modelling the column cannot resolve, and the
-- posting job would then carry the short form into transaction.tran_source, which is also PIC X(10).
-- The two literals below therefore each carry their two trailing blanks, and the verification block
-- at the end of this file asserts the width and the vocabulary so the two artefacts cannot drift
...
--       [18:22] dalytran_cat_cd        [22:32] dalytran_source (padded, preserved)
...
DECLARE
    ...
    v_source_width      bigint;
    v_source_vocabulary bigint;
...
BEGIN
    ...
    --   width     - every value is exactly the 101 characters a column-bound twenty-character
    --               payload produces, so no literal was truncated by a wrapped line or a stray
    --               quote, AND no literal was seeded without its column binding: an unbound payload
    --               measures 69, so the width check is also the binding check this block can make
    ...
    SELECT count(*) INTO v_govt_width
      FROM customer WHERE length(govt_issued_id) = 101;
    ...
    IF v_govt_width <> 50 THEN
        RAISE EXCEPTION 'V3 seed: % sealed identifiers measure 101 characters, expected 50 - a'
            ' different width means a literal was altered, truncated, or seeded without the column'
            ' binding every reader of this column opens it under', v_govt_width;
    END IF;
    ...
    -- Daily composition: 250 purchases and 50 returns, matching sign for matching source. The two
    -- literals carry their two trailing blanks because the column is a coded field seeded at the
    -- layout width - see the trim rule in the header and the section preamble above. Written without
    -- btrim on either side deliberately: a trimming comparison here would keep passing if a future
    -- edit re-shortened the stored values, which is exactly the drift these counts must catch.
    SELECT count(*) INTO v_source_pos_term
      FROM daily_transaction WHERE dalytran_source = 'POS TERM  ';
    SELECT count(*) INTO v_source_operator
      FROM daily_transaction WHERE dalytran_source = 'OPERATOR  ';
    ...
    IF v_source_pos_term <> 250 OR v_source_operator <> 50 THEN
        RAISE EXCEPTION 'V3 seed: daily sources hold % point-of-sale and % operator rows at the'
            ' padded ten-character width, expected 250 and 50',
            v_source_pos_term, v_source_operator;
    END IF;
    ...
    -- Two further checks on the same column, because the counts above would still pass if the two
    -- spellings were swapped for two other ten-character values, and they are the checks that keep
    -- the seed, the record layout, the record mapper and the enumeration in agreement:
    --   width      - every stored value occupies the whole PIC X(10) field, so no row carries the
    --                right-trimmed eight-character spelling display text would have received;
    --   vocabulary - every stored value is one of the three the enumeration declares, so no row
    --                carries a padded value from outside the closed set either.
    SELECT count(*) INTO v_source_width
      FROM daily_transaction WHERE length(dalytran_source) = 10;
    SELECT count(*) INTO v_source_vocabulary
      FROM daily_transaction
     WHERE dalytran_source IN ('POS TERM  ', 'OPERATOR  ', 'System    ');
    IF v_source_width <> 300 THEN
        RAISE EXCEPTION 'V3 seed: % daily rows carry a ten-character source, expected 300 - a'
            ' right-trimmed value here is a value the source enumeration cannot resolve',
            v_source_width;
    END IF;
    IF v_source_vocabulary <> 300 THEN
        RAISE EXCEPTION 'V3 seed: % daily rows carry a source inside the declared vocabulary,'
            ' expected 300', v_source_vocabulary;
    END IF;
