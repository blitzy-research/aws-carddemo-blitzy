...
--
-- APPLIES TO ALL PROFILES. All four delivered scripts sit FLAT in classpath:db/migration, the one
-- location EVERY profile resolves, so no profile can start without V1. Sample rows and sign-on
-- identities are V3 and V4, and what holds them out of production is the VERSION CEILING:
-- spring.flyway.target is pinned to 2 in the shared baseline and re-asserted by the production overlay
-- and by FlywayConfig, so V3 and V4 are discovered, reported as above-target, and never executed there.
-- There is no schema/ or seed/ child directory under db/migration and none may be created - a Flyway
-- location is scanned recursively, so a child directory would be reached by any profile that resolves
-- the parent and could separate nothing. A production migration therefore inherits schema and indexes
-- and nothing else. This file inserts no row of any kind.
--
