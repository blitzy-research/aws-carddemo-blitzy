...
-- nine tables, and nothing else. This file ships from classpath:db/migration/seed, a SIBLING of the
-- schema location and not a child of it, and what holds it out of production is exactly that: production
-- declares classpath:db/migration/schema alone, so this script and V4 are not applied, not pending and
-- NOT RESOLVED AT ALL there. Only the local and test overlays add the seed location, which is the single
-- setting that makes this file executable. See PROFILE APPLICABILITY below:
--
...
--
-- The boundary is drawn by LOCATION, and three independent controls hold it:
--
--   THE SEPARATION IS THE LOCATION LIST. The two schema scripts and the two seeds ship from sibling
--   directories, and their shared parent holds no script at all:
--
--       classpath:db/migration            -> holds NO script; REFUSED as a location by every profile,
--                                            because a location is scanned RECURSIVELY and the parent
--                                            would reach both children
--       classpath:db/migration/schema     -> declared by EVERY profile, production included
--           V1__create_schema.sql             \
--           V2__create_indexes.sql             >  APPLIED everywhere
--       classpath:db/migration/seed       -> declared by the local and test overlays ONLY
--           V3__seed_reference_data.sql       \  not resolved under production at all: absent from
--           V4__seed_user_security.sql         >  its migration state entirely
--
--   application.yml declares the SCHEMA LOCATION ALONE as the shared default, so a profile that stays
--   silent inherits a schema-only migration rather than an unnoticed seeding run, and
--   application-prod.yml re-states it so the production posture is legible in the file that governs it.
--   The local and test overlays add the seed location, which is the single setting that applies this
...
--   parent classpath:db/migration anywhere - it is refused under every profile because it reaches both
--   children and records each script under a name relative to itself. Any further seed script belongs in
--   this directory and must carry a version above every schema version, because Flyway orders by version
--   across all resolved locations and a lower-numbered seed would be applied before the table it fills.
--
--   AND THE LOCATION LIST IS CHECKED IN CODE, WHICH IS WHY A CONFIGURATION EDIT ALONE CANNOT REACH THIS
--   FILE. com.carddemo.config.FlywayConfig inspects the resolved list AFTER it is bound, and under the
