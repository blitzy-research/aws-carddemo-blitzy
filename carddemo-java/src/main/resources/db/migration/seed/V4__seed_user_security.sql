...
--
-- THIS FILE SHIPS FROM A LOCATION PRODUCTION DOES NOT RESOLVE. The two schema scripts ship from
-- classpath:db/migration/schema, which every profile declares; this file and V3 ship from the SIBLING
-- classpath:db/migration/seed, which only the local and test overlays declare. Their shared parent
-- classpath:db/migration holds no script and is refused as a location under every profile, because a
-- Flyway location is scanned RECURSIVELY and the parent would reach both children. Do not move this file
-- into the schema location or into the parent, and do not declare the parent anywhere.
--
-- HOW THE EXCLUSION IS ENFORCED - TWO CONTROLS: the PROFILE-SCOPED LOCATION LIST, and refusals in code
-- that read it back after binding and additionally inspect the database itself. Both are recorded in
-- docs/decision-log.md DL-298, which re-supersedes DL-102, DL-108, DL-111, DL-116, DL-119 and DL-127.
--
--   CONTROL 1 - THE LOCATION LIST. The two halves ship from sibling directories, and the difference
--   between a production migration and a seeding one is which of them the profile declares:
--
...
--            V3__seed_reference_data.sql        \  NOT RESOLVED under production: absent from its
--            V4__seed_user_security.sql (this)   >  migration state entirely
--
...
--   applied before the table it fills. Do not add this location to the base or production profile, and
--   never reintroduce a numeric target as a substitute for the list.
--
--   CONTROL 2 - THE REFUSALS IN CODE, which cover a deployment whose configuration was edited, merged
...
--   com.carddemo.config.FlywayConfig inspects the BOUND location list and, under the production profile,
--   refuses any list that is not exactly classpath:db/migration/schema, matched by EQUALITY rather than
--   containment - so a file-system look-alike, a bare path, a deeper sub-path, this file's own location,
--   the shared parent, a second entry and a blank entry are each refused. It additionally refuses a
--   NUMERIC target under production, so the withdrawn ceiling cannot return as a trap that silently
--   freezes the schema.
...
--   control 1 is explicit and reviewable but is a property of configuration, and the
...
--
-- A row in user_security in a production database means a location list was widened or the database was
-- seeded elsewhere first. Treat it as an incident, not as drift.
--
