...
public final class JobParameterValidators {
    ...
    // =================================================================================================
    // The staged-input location contract. A location is the ONE kind of launch parameter this module
    // accepts that is afterwards handed to a resource resolver, so it is the one that needs an
    // allow-list rather than a format check: a resolver understands schemes this module has no business
    // reading from, and a launch parameter arrives over HTTP.
    // =================================================================================================

    /**
     * Scheme prefix of an object-store location, which is the first of the two accepted forms.
     *
     * <p>Published so that a job configuration, a diagnostic and a test all spell it once. The comparison
     * against it is case-insensitive and the canonical form this class returns is lower case, because a
     * resolver matches the scheme case-insensitively and an allow-list that did not would be bypassable
     * by capitalising one letter.</p>
     */
    public static final String OBJECT_STORE_SCHEME = "s3://";

    /**
     * Longest object key the allow-list accepts, which is the object store's own key limit.
     *
     * <p>A bound rather than a tuning figure: a longer value is not a key the store could hold, and
     * scanning an unbounded one for the characters below is work spent on an input that cannot succeed.</p>
     */
    private static final int MAX_OBJECT_KEY_LENGTH = 1024;

    /** Path separator inside an object key and inside a staged name, in both accepted forms. */
    private static final char LOCATION_SEPARATOR = '/';

    /**
     * The parent-directory segment, refused wherever it appears in either accepted form.
     *
     * <p>Refused by segment rather than by substring, so a legitimate name carrying two dots inside a
     * component - which the legacy dataset names do, repeatedly - is not caught by the rule that exists to
     * stop traversal.</p>
     */
    ...
    /**
     * Returns the validator for one or more staged-input location parameters.
     *
     * <p><strong>Why this contract exists at all.</strong> Every other launch parameter this class governs
     * is read as a value - a date, a mode - and a format check is the whole of its contract. A location is
     * different in kind: it is afterwards handed to a resource resolver, and a resolver will happily open
     * whatever scheme it recognises. Left unchecked, a launch parameter arriving over HTTP therefore selects
     * what the process reads: a local file outside the module's own staging area, an address on the network
     * the process can reach, an entry inside an archive on the class path. The records so read are then
     * interpreted at a fixed width and loaded into the transaction master, so the same parameter also
     * chooses what is written. A format check cannot close that; only an allow-list can.</p>
     *
     * <p><strong>The allow-list is two forms and nothing else.</strong> An object key beneath the one
     * configured staging bucket, written either bare or as {@value #OBJECT_STORE_SCHEME} followed by that
     * bucket and the key; or a relative name beneath one canonical base directory, which is the staging
     * directory the job itself is configured with. Every other input is refused by name: another bucket,
     * any other scheme, an absolute path, and any value whose path would leave the canonical base after
     * normalisation.</p>
     *
     * <p>The rejection is reported through the framework's own contract, so a refused launch is answered as
     * an invalid parameter set rather than as an internal failure, and the offending value is rendered
     * through the same guard every other diagnostic in this class uses. The check is deliberately performed
     * <strong>twice</strong>: here, at the launch boundary, where a refusal reaches the caller as a bad
     * request; and again by {@link #requireStagedInputLocation(String, String, String, String)} at the point
     * the location is resolved, because a step-scoped resolution can also be reached by a job launched
     * around this validator and a boundary check that can be bypassed is not a boundary.</p>
     *
     * @param  parameterKeys  the location parameter keys this job declares, in report order; must be
     *                        non-empty and carry no null, blank or non-printable entry
     * @param  stagingBucket  name of the one object-store bucket a location may name; must not be blank
     * @param  canonicalBase  the one directory a relative name is resolved against; must not be blank
     * @return a stateless validator, never {@code null}
     * @throws NullPointerException     if any argument is {@code null}
     * @throws IllegalArgumentException if {@code parameterKeys} is empty or carries an unusable entry, or
     *                                  if either of the two boundaries is blank
     */
    public JobParametersValidator stagedInputLocationValidator(final Collection<String> parameterKeys,
                                                               final String stagingBucket,
                                                               final String canonicalBase) {
        final Set<String> keys = copyLocationParameterKeys(parameterKeys);
        final String bucket = requireBoundary(stagingBucket, "stagingBucket");
        final String base = requireBoundary(canonicalBase, "canonicalBase");
        return jobParameters -> validateStagedInputLocations(jobParameters, keys, bucket, base);
    }

    /**
     * Canonicalises one staged-input location, refusing anything outside the two accepted forms.
     *
     * <p>This is the resolution-time half of the contract described on
     * {@link #stagedInputLocationValidator(Collection, String, String)}. It answers a location a resource
     * resolver may safely be handed: either {@value #OBJECT_STORE_SCHEME} followed by the configured bucket
     * and a validated key, or a file address inside the canonical base. The returned value is composed
     * here from the validated components rather than being the caller's text with a scheme prepended, so
     * nothing that was refused can be reconstructed from what is returned.</p>
     *
     * <p>It is {@code static} because it needs no instance state, and it reports through
     * {@link IllegalArgumentException} rather than the framework's checked contract because its callers are
     * resolution sites rather than validators - a job that reached resolution with an unusable location is
     * a launch that should never have been accepted, and the two paths must not be reported the same way.</p>
     *
     * @param  location      the location the job parameter carried, possibly {@code null} or blank
     * @param  parameterKey  the parameter key, named in the refusal
     * @param  stagingBucket name of the one object-store bucket a location may name
     * @param  canonicalBase the one directory a relative name is resolved against
     * @return the canonical, resolvable location, never {@code null}
     * @throws NullPointerException     if {@code parameterKey}, {@code stagingBucket} or
     *                                  {@code canonicalBase} is {@code null}
     * @throws IllegalArgumentException if the location is absent, blank, or outside the two accepted forms
     */
    public static String requireStagedInputLocation(final String location, final String parameterKey,
                                                    final String stagingBucket,
                                                    final String canonicalBase) {
        Objects.requireNonNull(parameterKey, "parameterKey must not be null");
        final String bucket = requireBoundary(stagingBucket, "stagingBucket");
        final String base = requireBoundary(canonicalBase, "canonicalBase");

        final StagedInputLocation resolved = canonicaliseStagedInputLocation(location, bucket, base);
        if (resolved.rejected()) {
            LOG.error("Job parameter [{}] named a location outside the staged-input allow-list: {}",
                    renderSafely(parameterKey), resolved.reason());
            throw new IllegalArgumentException("Job parameter [" + renderSafely(parameterKey) + "] "
                    + resolved.reason());
        }
        LOG.debug("Staged-input location accepted for job parameter [{}]", renderSafely(parameterKey));
        return resolved.canonicalLocation();
    }
    ...
    /**
     * Validates every declared staged-input location against the allow-list, in declaration order.
     *
     * <p>Static because it needs no instance state: both boundaries and the key set arrive as arguments,
     * closed over by the lambda the accessor returned, and no calendar decision is involved.</p>
     *
     * <p>Each key is checked and reported on its own, in the order the job declared them, so a launch that
     * names two unusable locations reports the first in that fixed order rather than whichever one a map
     * happened to yield first.</p>
     *
     * @param jobParameters the parameter set the job was launched with
     * @param parameterKeys the location keys this job declares, in report order
     * @param stagingBucket name of the one object-store bucket a location may name
     * @param canonicalBase the one directory a relative name is resolved against
     * @throws JobParametersInvalidException if the parameter set is absent, or any declared location is
     *                                       missing, blank or outside the two accepted forms
     */
    private static void validateStagedInputLocations(final JobParameters jobParameters,
                                                     final Set<String> parameterKeys,
                                                     final String stagingBucket,
                                                     final String canonicalBase)
            throws JobParametersInvalidException {
        ...
        for (final String parameterKey : parameterKeys) {
            final StagedInputLocation resolved = canonicaliseStagedInputLocation(
                    supplied.getString(parameterKey), stagingBucket, canonicalBase);
            if (resolved.rejected()) {
                throw new JobParametersInvalidException("Job parameter [" + renderSafely(parameterKey)
                        + "] " + resolved.reason());
            }
        }
        LOG.debug("Staged-input locations accepted for {}", parameterKeys);
    }

    /**
     * Applies the staged-input allow-list to one location, answering either its canonical form or the
     * reason it was refused.
     *
     * <p>One method, so that the launch-boundary validator and the resolution-time guard cannot come to
     * disagree about what is acceptable. It never throws for a rejected input - the caller's own contract
     * decides how a refusal is reported - and it never returns the caller's text: an accepted location is
     * recomposed from the components that were validated.</p>
     *
     * <p>The order of the checks is the contract. Presence first, so an absent value is reported as absent
     * rather than as malformed. Representability second, because a location reaches a diagnostic and a
     * log record and a control character in it would forge one. Then the two accepted forms, tried in the
     * order they are declared, with everything else refused by naming what the two forms are.</p>
     *
     * @param  location      the location to check, possibly {@code null} or blank
     * @param  stagingBucket name of the one object-store bucket a location may name, never blank
     * @param  canonicalBase the one directory a relative name is resolved against, never blank
     * @return an accepted result carrying the canonical location, or a rejected result carrying the reason
     */
    private static StagedInputLocation canonicaliseStagedInputLocation(final String location,
                                                                      final String stagingBucket,
                                                                      final String canonicalBase) {
        if (location == null) {
            return StagedInputLocation.rejected("is required but was not supplied; it must name an object"
                    + " key in bucket [" + renderSafely(stagingBucket) + "] or a name beneath the"
                    + " configured staging directory, and no default may be substituted for it");
        }
        if (location.isBlank()) {
            return StagedInputLocation.rejected("is blank; it must name an object key in bucket ["
                    + renderSafely(stagingBucket) + "] or a name beneath the configured staging directory");
        }
        if (firstNonPrintableIndex(location) >= 0) {
            return StagedInputLocation.rejected("must not hold a character outside printable US-ASCII,"
                    + " because a location is written to the batch log and no acceptable location carries"
                    + " one; the supplied value is " + describeValue(location));
        }

        final String trimmed = location.strip();
        if (trimmed.regionMatches(true, 0, OBJECT_STORE_SCHEME, 0, OBJECT_STORE_SCHEME.length())) {
            return objectStoreLocation(trimmed.substring(OBJECT_STORE_SCHEME.length()), stagingBucket);
        }
        final int schemeEnd = schemeDelimiterIndex(trimmed);
        if (schemeEnd >= 0) {
            return StagedInputLocation.rejected("names the scheme ["
                    + renderSafely(trimmed.substring(0, schemeEnd)) + "], which this job will not read"
                    + " from; the only accepted forms are an object key in bucket ["
                    + renderSafely(stagingBucket) + "], optionally written with the "
                    + OBJECT_STORE_SCHEME + " prefix, and a relative name beneath the configured staging"
                    + " directory");
        }
        return stagedFileLocation(trimmed, stagingBucket, canonicalBase);
    }

    /**
     * Applies the allow-list to the object-store form, whose bucket must be the configured one.
     *
     * <p>The bucket is compared exactly and the key is then held to the object store's own key grammar.
     * Both halves matter: a permitted key in someone else's bucket is still someone else's data, and a
     * permitted bucket with a traversing key is the same defect written differently.</p>
     *
     * @param  bucketAndKey  everything after the scheme prefix
     * @param  stagingBucket name of the one bucket a location may name
     * @return an accepted result carrying {@code s3://bucket/key}, or a rejected result with the reason
     */
    private static StagedInputLocation objectStoreLocation(final String bucketAndKey,
                                                           final String stagingBucket) {
        final int separator = bucketAndKey.indexOf(LOCATION_SEPARATOR);
        if (separator < 0 || separator == bucketAndKey.length() - 1) {
            return StagedInputLocation.rejected("names no object key; the object-store form is "
                    + OBJECT_STORE_SCHEME + renderSafely(stagingBucket) + "/ followed by the key");
        }
        final String bucket = bucketAndKey.substring(0, separator);
        if (!stagingBucket.equals(bucket)) {
            return StagedInputLocation.rejected("names bucket [" + renderSafely(bucket) + "], and this job"
                    + " reads only from the configured staging bucket [" + renderSafely(stagingBucket)
                    + "]");
        }
        final String key = bucketAndKey.substring(separator + 1);
        final Optional<String> keyRejection = objectKeyRejection(key);
        if (keyRejection.isPresent()) {
            return StagedInputLocation.rejected(keyRejection.get());
        }
        return StagedInputLocation.accepted(OBJECT_STORE_SCHEME + stagingBucket + LOCATION_SEPARATOR + key);
    }

    /**
     * Applies the allow-list to the relative-name form, which must stay inside the canonical base.
     *
     * <p>A bare name is treated as an object key first and as a path second, and both readings have to
     * hold: the key grammar rejects the character classes a path check would let through, and the
     * normalisation test rejects the traversals a grammar check cannot see. Containment is asserted on the
     * <em>normalised</em> path, because that is the path that would actually be opened.</p>
     *
     * @param  name          the relative name, already trimmed, carrying no scheme
     * @param  stagingBucket name of the bucket, named in a refusal so the message states both forms
     * @param  canonicalBase the one directory the name is resolved against
     * @return an accepted result carrying a file address inside the base, or a rejected result
     */
    private static StagedInputLocation stagedFileLocation(final String name, final String stagingBucket,
                                                           final String canonicalBase) {
        final Optional<String> keyRejection = objectKeyRejection(name);
        if (keyRejection.isPresent()) {
            return StagedInputLocation.rejected(keyRejection.get());
        }
        if (name.charAt(0) == LOCATION_SEPARATOR || name.charAt(0) == '\\') {
            return StagedInputLocation.rejected("is an absolute path; this job reads only a relative name"
                    + " beneath its configured staging directory, or an object key in bucket ["
                    + renderSafely(stagingBucket) + "]");
        }

        final Path base;
        final Path resolved;
        try {
            base = Path.of(canonicalBase).toAbsolutePath().normalize();
            resolved = base.resolve(name).normalize();
        } catch (final InvalidPathException unusable) {
            return StagedInputLocation.rejected("is not a usable name beneath the configured staging"
                    + " directory: " + describeValue(name));
        }
        if (Path.of(name).isAbsolute() || !resolved.startsWith(base) || resolved.equals(base)) {
            return StagedInputLocation.rejected("resolves outside the configured staging directory; this"
                    + " job reads only a relative name beneath it, or an object key in bucket ["
                    + renderSafely(stagingBucket) + "]");
        }
        return StagedInputLocation.accepted(resolved.toUri().toString());
    }

    /**
     * Holds a key or relative name to the object store's own key grammar.
     *
     * <p>The grammar is the store's safe-character set plus the separator, which is narrow enough to
     * exclude every shell, query and fragment character while still admitting every legacy dataset name
     * this module stages - those carry letters, digits, dots and hyphens only. The parent segment is
     * refused by segment rather than by substring, so a name whose component legitimately contains two
     * dots is unaffected.</p>
     *
     * @param  key the key or relative name to check, never {@code null}
     * @return the reason it is unacceptable, or empty when it satisfies the grammar
     */
    private static Optional<String> objectKeyRejection(final String key) {
        if (key.isEmpty()) {
            return Optional.of("names an empty object key");
        }
        if (key.length() > MAX_OBJECT_KEY_LENGTH) {
            return Optional.of("names an object key of " + key.length() + " characters, and the object"
                    + " store holds no key longer than " + MAX_OBJECT_KEY_LENGTH);
        }
        for (int index = 0; index < key.length(); index++) {
            if (!isObjectKeyCharacter(key.charAt(index))) {
                return Optional.of("holds a character an object key may not carry at position " + index
                        + "; an accepted location carries letters, digits and the characters"
                        + " ! - _ . * ' ( ) / only");
            }
        }
        for (final String segment : key.split(String.valueOf(LOCATION_SEPARATOR), -1)) {
            if (PARENT_SEGMENT.equals(segment)) {
                return Optional.of("carries a parent-directory segment, which would let it name something"
                        + " outside the staging area it is resolved against");
            }
        }
    ...
    /**
     * Whether one character belongs to the object store's safe key set, plus the separator.
     *
     * @param  character the character to classify
     * @return {@code true} when a key may carry it
     */
    private static boolean isObjectKeyCharacter(final char character) {
        return (character >= 'a' && character <= 'z')
                || (character >= 'A' && character <= 'Z')
                || (character >= '0' && character <= '9')
                || character == '!' || character == '-' || character == '_' || character == '.'
                || character == '*' || character == '\'' || character == '(' || character == ')'
                || character == LOCATION_SEPARATOR;
    }

    /**
     * The index of the colon ending a leading URI scheme, or {@code -1} when the value carries none.
     *
     * <p>Scanned rather than delegated to a URI parser, because a parser accepts values this check has to
     * refuse and rejects values it has to accept: a legacy dataset name is not a URI, and a scheme this
     * module will not read from must be recognised even when the rest of the value is malformed. The
     * grammar is the one the addressing standard states - a letter, then letters, digits, plus, minus or
     * dot, then a colon - and a Windows drive letter is deliberately caught by it, since a drive-qualified
     * path is an absolute path.</p>
     *
     * @param  value the trimmed value to scan, never empty
     * @return the index of the scheme's colon, or {@code -1} when there is no leading scheme
     */
    private static int schemeDelimiterIndex(final String value) {
        ...
        for (int index = 1; index < value.length(); index++) {
            ...
            final boolean schemeCharacter = (character >= 'a' && character <= 'z')
                    || (character >= 'A' && character <= 'Z')
                    || (character >= '0' && character <= '9')
    ...
    /**
     * Copies the caller's location parameter keys into an unmodifiable, order-preserving set.
     *
     * <p>The same construction-time refusals {@link #copyLegalModeNames(Collection)} applies, and for the
     * same reasons: a validator built from an empty or unusable declaration would refuse launches it was
     * configured to accept, and every key is reproduced inside the diagnostic of a rejected launch.</p>
     *
     * @param  parameterKeys the caller's collection
     * @return an unmodifiable copy in declaration order, never empty
     * @throws NullPointerException     if {@code parameterKeys} is {@code null}
     * @throws IllegalArgumentException if the collection is empty or carries a null, blank or
     *                                  non-printable entry
     */
    private static Set<String> copyLocationParameterKeys(final Collection<String> parameterKeys) {
        Objects.requireNonNull(parameterKeys, "parameterKeys must not be null: the job configuration owns"
                + " its location parameter keys and supplies them here");

        final Set<String> copy = new LinkedHashSet<>();
        for (final String parameterKey : parameterKeys) {
            if (parameterKey == null || parameterKey.isBlank()) {
                throw new IllegalArgumentException("parameterKeys must not carry a null or blank entry,"
                        + " because no launch parameter could ever be read under one");
            }
            if (firstNonPrintableIndex(parameterKey) >= 0) {
                throw new IllegalArgumentException("parameterKeys must not carry a character outside"
                        + " printable US-ASCII, because every declared key is reproduced inside the"
                        + " diagnostic of a rejected launch; the offending entry is "
                        + describeValue(parameterKey));
            }
            copy.add(parameterKey);
        }
        if (copy.isEmpty()) {
            throw new IllegalArgumentException("parameterKeys must name at least one location parameter,"
                    + " because a validator over no key would check nothing while appearing to");
        }
        return Collections.unmodifiableSet(copy);
    }

    /**
     * Refuses a blank allow-list boundary, because a validator built on one would admit anything.
     *
     * <p>Failed at configuration time rather than at launch time. A blank bucket would make every bucket
     * comparison fail and refuse every launch; a blank base would resolve a relative name against the
     * process's working directory, which is not a boundary at all. Both are configuration defects, and
     * failing where the mistake was made is more useful than failing where it is felt.</p>
     *
     * @param  boundary the configured value
     * @param  name     the argument's name, for the diagnostic
     * @return the boundary as supplied
     * @throws NullPointerException     if {@code boundary} is {@code null}
     * @throws IllegalArgumentException if {@code boundary} is blank
     */
    private static String requireBoundary(final String boundary, final String name) {
        Objects.requireNonNull(boundary, () -> name + " must not be null: it is one boundary of the"
                + " staged-input allow-list");
        if (boundary.isBlank()) {
            throw new IllegalArgumentException(name + " must not be blank: a blank boundary is not a"
                    + " boundary, and an allow-list built on one either refuses everything or admits"
                    + " everything");
        }
        return boundary;
    }
    ...
    /**
     * The outcome of applying the staged-input allow-list to one location.
     *
     * <p>A two-state carrier rather than an exception, so that the launch-boundary validator and the
     * resolution-time guard can raise the exception their own contract requires from one identical
     * sequence of checks. Exactly one of the two components is populated, which the two factory methods
     * guarantee and which {@link #rejected()} is read through.</p>
     *
     * @param canonicalLocation the canonical, resolvable location when the value was accepted, otherwise
     *                          {@code null}
     * @param reason            the reason the value was refused when it was refused, otherwise
     *                          {@code null}
     */
    private record StagedInputLocation(String canonicalLocation, String reason) {

        /**
         * @param  canonicalLocation the recomposed location, never {@code null}
         * @return an accepted outcome
         */
        static StagedInputLocation accepted(final String canonicalLocation) {
            return new StagedInputLocation(canonicalLocation, null);
        }

        /**
         * @param  reason the reason the location was refused, never {@code null}
         * @return a rejected outcome
         */
        static StagedInputLocation rejected(final String reason) {
            return new StagedInputLocation(null, reason);
        }

        /** @return whether the location was refused */
        boolean rejected() {
