...
/**
 * Immutable per-card statement transaction projection, owned by the service layer.
 *
 * <p>This is the service-tier carrier for the reporting-altered transaction record declared by
 ...
 *
 * <p><strong>Why this exists beside {@code api/dto/StatementSummary}.</strong> The two are twins of
 * the same copybook, component for component and name for name. The transport record stays in the
 * API layer because that is where the published schema belongs, and this one exists because the
 * statement generator that builds it sits in the service layer, which may not name a transport type:
 * the strict downward package direction the module is built on allows {@code api -> service} and
 * forbids the reverse. Duplicating a thirteen-component projection is the price of that direction,
 * and it is paid here rather than by relaxing the direction, because the projection is pure data with
 * no behaviour to keep in step. Neither twin may acquire a component, a width or a rendering rule the
 * other lacks.</p>
 *
 ...
 * the image out to 350 bytes. No component or accessor corresponds to it, so a caller can neither
 * observe nor supply it.</p>
 *
 ...
 * {@code com.carddemo.util.ZonedDecimalCodec}. Keeping the policy in one class is what stops two
 * truncation behaviours coexisting, and truncation rather than nearest-value rounding is faithful
 * because no arithmetic statement anywhere in the legacy estate asks for rounding (decision log entry
 * D-02). This type performs no arithmetic, scaling, negation or sign handling; it carries the value it
 * was handed.</p>
 *
 ...
 * @since 1.0.0
 */
public record StatementLineSummary(
        String cardNumber,
    ...
    // constructor, no defaulting and no normalisation. Every value crosses into the statement writers
    // byte for byte, which is what lets a 26-blank stamp, a blank-filled channel value and a
    // leading-zero code reach them unchanged.
     ...
     * account number. This is the twin that the statement generator actually constructs - once per
     * statement line, inside the batch tier - so a default rendering would place a primary account
     * number one interpolation away from every log line, assertion message and diagnostic dump on the
     * statement path, which is the highest-volume rendering surface in the module. The transport twin
     * redacts for the same reason; both must, because either one reaching a log is the same disclosure.
     *
     ...
     *
     * <p>The rendering names this type rather than its transport twin, so a diagnostic says which side of
     * the boundary the value was on. {@code equals} and {@code hashCode} remain as the record contract
     * generates them: they compare every component by value and emit nothing, so byte-for-byte fixture
