 ...
 * one line terminator each. Those rows reach the local and test profiles only: the seeding migration is
 * delivered from {@code classpath:db/migration/seed}, a location a production deployment is refused and
 * therefore never resolves, and a production migration additionally stops after the index script, so a
 * production database receives the schema and the indexes without the sample rows.
