 ...
 * one line terminator each. Those rows reach the local and test profiles only: every migration is
 * delivered from one flat classpath location and a production deployment stops its migration after the
 * index script, so a production database receives the schema and the indexes without the sample rows.
