 ...
 *
 * <p>The version fallback is the subtle part. Build information is optional at runtime: it exists
 * only when the packaging step generates it, so a plain {@code java -jar} run of a locally compiled
 * class tree has none. The configuration must publish a usable version in both cases, and must not
 * publish the literal absence.
 */
...
final class ContractPublicationConfigTest {
    ...
    final class PublishedContract {
        ...
        void anUnstampedBuildFallsBackToTheModuleCoordinate() {
            // Build information is generated by the packaging step, so a locally compiled class tree
            // has none. Publishing nothing would leave consumers without a contract version at all.
