...
| ---: | :--------------------------------- | :--------------------------------------------------------------------------------------------- |
| 1 | End-to-end byte-equivalent output | `BatchPipelineE2ETest` compares produced files against golden fixtures as **byte arrays, never semantically**, at the **four contractual widths** this gate is frozen at — **80, 100, 133 and 430 bytes** — all four from one seeded pipeline pass. The 40-byte category-balance listing is compared against its own golden at its emitting job, as **supplemental** evidence beside that criterion rather than as a member of that frozen set |
...
| 8 | Integration sign-off | OWASP dependency-check bound to `verify` and actually executed, failing the build at CVSS **7.0**. The exact recorded result, rather than a round claim: **zero unsuppressed critical**, **zero unsuppressed high**, **one high finding covered by a written analyst determination** (`CVE-2026-66299`, CVSS 7.5, matched on `tomcat-embed-core`, scoped to that single identifier and self-expiring because `failBuildOnUnusedSuppressionRule` fails the build the moment it stops matching), and **one visible below-threshold medium** (`CVE-2026-41178`, CVSS 5.3, on `opentelemetry-semconv` — reported and not suppressed). Both are disclosed with their dated report in [`docs/gate-evidence.md`](docs/gate-evidence.md), alongside three records that are **closed by a version move rather than accepted**: `CVE-2026-64607` on `httpclient5`, which this page previously listed as a second below-threshold medium and which moved the moment its fixed release was published; and `CVE-2026-13006` (7.0, logback) and `CVE-2026-45292` (baggage propagation, OpenTelemetry), **neither of which this scan reported at all** because each record carries no CPE configuration for a CPE-matching scanner to compare against. One further coordinate is **version-affected and carried as a dated determination** rather than fixed or suppressed: `CVE-2026-44308` on `spring-cloud-aws-sns`, whose affected range ends at the resolved 3.4.2, whose only fix is a release built on the Spring Boot 4 line the plan's 3.x stack rules out, and whose inbound notification surface this module does not implement at all. **A CVE result ages even when the code does not**, and a green scan is necessary rather than sufficient: re-run the scan, and read the evidence page's dependency section, before any sign-off rather than quoting this row. **A second supply chain is gated on the same terms and reported separately**: the container gate scans the application image, both `Dockerfile` bases and all five third-party Compose images at HIGH and CRITICAL with no filter of any kind. The artefact this module ships is clean — **0 findings** on the application image and on both Temurin bases — and three supporting pins were moved on measurement rather than assumption, taking the local validation stack from **531 findings with 61 critical to 315 with 44**. Two pins refused to move and the reasons differ: PostgreSQL cannot leave `16.14-bookworm` without changing collation ordering underneath the parity fixtures, and the emulator cannot leave `4.14.0` because the newer, far cleaner image is **not the Community edition** and demands a paid auth token — a scanner never runs the entrypoint, so *measurably cleaner* and *usable* are different questions. The remaining 177 findings are carried as dated, attributed, per-identifier determinations in `carddemo-java/container-scan-determinations.txt`, which the gate reads and which fails the build when a determination expires or stops matching. Plus the **544-row** traceability matrix covering every procedure unit |
...
| OpenTelemetry OTLP exporter | `1.62.0` — pinned above the `1.49.0` the Spring Boot 3.5.16 bill of materials manages, for two reasons. It is the fixed release for `CVE-2026-45292`, unbounded allocation in W3C baggage propagation, which affects the deployed `opentelemetry-api` and `opentelemetry-extension-trace-propagators` below 1.62.0 and which the CPE-matching dependency scan cannot see because the record carries no CPE configuration at all. It also keeps the floor the earlier pin was written for: on 1.49.0 the span processor's shaded queue takes field offsets through `sun.misc.Unsafe` and the runtime writes three unstructured warnings to the error stream at every start-up, and 1.62.0 was read artefact-by-artefact to confirm it carries no such reference either |
| `logstash-logback-encoder` | `9.0` |
| Logback classic / SLF4J API | `1.5.38` / `2.0.18` — logback is pinned above the `1.5.34` the Spring Boot 3.5.16 bill of materials manages, for `CVE-2026-13006`: arbitrary code execution through Janino-backed conditional processing of a configuration file, affecting every release up to and including 1.5.36 and reported by neither the dependency scan (the record publishes no CPE configuration) nor the advisory database. 1.5.37 removed that processing outright; 1.5.38 additionally repairs a hardened-deserialisation filter |
