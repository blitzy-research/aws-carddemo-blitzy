...
| Property | Value |
| --- | --- |
| Date, in UTC | 2026-08-13, build finished 23:01:54Z |
| Command | `./mvnw -B clean verify`, run from the module directory |
| Result | `BUILD SUCCESS`, total time 12:21 min |
| Reproduced by | repeated full `./mvnw -B clean verify` runs on the same machine, each `BUILD SUCCESS`; every run over this tree produced identical standing results — the same 246 and 547 source counts, no compiler diagnostic of any kind, the same 27,123 and 1,666 test cases with no failure, the same 23,368 of 24,326 lines and 7,800 of 8,724 branches across the same 496 analysed classes, the same five byte-equal Gate 1 comparisons — its four contractual widths and the supplemental one — and the same 166 report entries covering 244 files from the supply-chain scan — the one figure in that set that is the **scanner's** own property rather than the module's, and which moved from 168 to 167 to 166 without any coordinate in the graph changing, as [Gate 8](#gate-8-integration-sign-off-checklist) records. Only the timings differed, which is the distinction this table exists to draw. **The vulnerability result is the exception and is dated for that reason**: the advisory feed moves independently of this tree, and across the runs recorded below a second below-threshold finding appeared in a dependency whose version did not change. It is published under [Gate 8](#gate-8-integration-sign-off-checklist) with its date rather than folded into this row |
...
| Sources compiled | **246** production and **547** test, each as `javac [debug parameters release 25]` under `-Xlint:all -Werror`, with no warning and no error |

**Why the two class counts differ from the two source counts, stated rather than smoothed over.** The
compiler compiled **547** test sources; the runners wrote **451** unit and **74** integration report files.
Three populations, not one, and each is correct for its own question. Nineteen of the 547 declare no test at
...
count are both 451 — and why that equality is what tells `e2e/GateVerificationTest` a run was unscoped
...
| 2 — Zero-warning build | **PASS (measured)** | `BUILD SUCCESS`; zero compiler warnings across **246** production and **547** test sources under `-Werror`; zero warning suppressions across both trees | `./mvnw -B clean verify` |
...
| 7 — Scope matching | **PASS (measured)** | merged line coverage 96.06% against a build-failing floor of 80% | `./mvnw -B clean verify` |
...
<div class="diagram__viewport" role="region" tabindex="0" aria-labelledby="diagram-gate-traceability-caption" markdown="1">
![Left to right graph tracing each of the eight gates back to what evidences it. The nine ASCII fixtures feed the Flyway V3 reference and transaction seed and the carried input fixtures, and the ten in-stream card images of the user-provisioning job feed the Flyway V4 identity seed. Those three feed the batch pipeline end-to-end test, which produces Gate 1 byte equivalence across four contractual widths, Gate 4 named artefacts, and the run-scoped performance recorder behind the Gate 3 baseline. The category-balance job integration test supplies the supplemental forty-byte width to Gate 1. The gate verification test supplies Gate 4, Gate 6 and Gate 8. The online transaction end-to-end test supplies Gate 5. The build file supplies Gate 2. The scoped grep audit supplies Gate 6. JaCoCo supplies Gate 7. The traceability matrix and the dependency check supply Gate 8, and Gates 1 through 7 all converge on Gate 8, which converges on this page.](diagrams/gate-traceability.svg){ .diagram__image width="1806" height="1229" }
</div>
<figcaption class="diagram__caption" id="diagram-gate-traceability-caption" markdown="1">
**Figure — Which artefact evidences which gate.** Every gate traced back to the artefact that evidences it. Gate 8 is the convergence point by construction, because its checklist is composed of the other seven, which is why a failure anywhere above surfaces there as well. Where this figure is wider than the page, the frame around it scrolls sideways; it can also be focused with the keyboard and panned with the arrow keys.
</figcaption>
</figure>
...
[INFO] Finished at: 2026-08-13T23:01:54Z
```

**Warning accounting, stated precisely rather than rounded off.** 246 production sources and 547 test
...
**Every repeat full `verify` reproduced this exactly**: the same 246 and 547 source counts, no compiler
...
| --- | ---: | ---: | ---: |
| `src/main/java` | 246 | **0** | 0 |
| `src/test/java` | 547 | **0** | 12 |
| Whole source | 793 | **0** (budget 3) | 12 |
...
code, and the audit honours that exception by covering every Java source the module actually contains: 246
under `src/main/java` and 547 under `src/test/java`, which is every `.java` file in the module outside
...
Rows 1 to 3 are the most recently measured run, taken on 2026-08-11 and finishing 22:44:42Z, and their five
figures are the ones that run's generated baselines carried. **They are deliberately not the run whose
transcript this page quotes under Gate 2**, and the distinction is the one drawn at the top of this page: a
standing figure — a source count, a test total, a coverage counter — is this tree's and is reconciled against
the build you just took, while a timing is a measurement of one run and belongs to that run permanently. This
table is held at thirty rows for that reason, so a later build cannot displace a measured row that a reader
can still attribute to the host and label beside it. Every build writes its own figures to
`target/gate-evidence/gate3-pipeline-baseline.md` and `target/gate-evidence/gate3-interest-calculation.md`,
which is where a timing for the build in front of you is read from rather than from this table; rows 4 to 6,
rows 7 to 9 and rows 10 to
...
| --- | ---: | ---: | --- |
| Raw SQL string concatenation | 0 | **0** | All access through Spring Data derived queries or parameterized JPQL. No `createNativeQuery` anywhere. **Four** advisory-lock statements are issued through a JDBC callback rather than a repository — `PostgresJobSubmissionCoordinator`, `AdvisoryGenerationPublicationLock`, `BatchLaunchCoordinator` and `TransactionInsertRepositoryImpl` — and each is a compile-time constant with its only variable bound as a parameter, as are the fixed statements the Flyway callbacks issue. Two earlier revisions of this row were wrong in opposite directions: one said "the one advisory-lock statement", which undercounted the sites, and one said five, which counted a sign-on attempt ledger withdrawn with the throttle it served (DL-352). Each revision stated the same true property of every site; the count now follows the tree. |
...
| Casts to a parameterised type, checked or not | ≤ 5 | **5** | The wider measure, published beside the narrower one so the two cannot be confused. All five are enumerated below; all five are checked. The cap was ≤ 6 and the count 6 while a shared sign-on attempt ledger acquired its advisory lock through the same `execute(ConnectionCallback)` idiom as the other lock sites; that ledger was withdrawn with the sign-on throttle it served (DL-352) and both the cap and the count follow the tree back down. What bounds the risk is the row above — unchecked casts, which `-Werror` holds at zero — not this count. |
...
**246** production sources of the tree this page ships with, so a reader can re-run each line and compare:
...
two in `service/SeededIdentifierSealingCallback`. The figure was **23** while the withdrawn sign-on attempt
ledger contributed six of its eight statement constants (DL-352); that it contributed six rather than eight
was the rule working rather than a miscount — a literal is counted only when one fragment carries both the
verb and a clause keyword. **Zero of the 17 is joined to a
...
`src/main/java` and `src/test/java` together — 793 sources — the raw `grep` returns **twelve** lines and the
...
emits a diagnostic — which is exactly why the unchecked count is zero while this count is five.

**A sixth existed and has gone, which is why the cap moved down rather than up.** The shared sign-on attempt
ledger — the store that made a sign-on attempt allowance deployment-wide rather than per-process — acquired a
PostgreSQL advisory lock exactly as the remaining lock sites do, through
`JdbcTemplate.execute(ConnectionCallback)`, and its cast selected that overload against
`execute(StatementCallback)`. That whole family was withdrawn as feature expansion: the legacy sign-on
transaction has no attempt counter, so the allowance was a behaviour the migration invented rather than one
it carried across. The cast went with the class, and the cap follows the tree back to five rather than being
left with headroom nothing occupies — a budget that admits a new site whenever an old one is removed measures
nothing, which is why `e2e/GateVerificationTest` also pins the exact set of classes permitted to carry one.
Recorded in `docs/decision-log.md` DL-352, with the withdrawn store's own reasoning at DL-343.
...
`.sql` schema artefacts, and an unscoped grep for SQL text over the module reports **five phantom raw-SQL
...
Five files, five phantom violations, and not one of them is application code assembling a statement at run
...
| **Recorded status** | **PASS (measured).** Merged line coverage 96.06% against a build-failing floor of 80%. |
...
| --- | ---: | ---: | ---: | :---: |
| **Line** | 23,368 | 24,326 | **96.06%** | **yes, floor 80%** |
| Branch | 7,800 | 8,724 | 89.41% | no |
| Instruction | 101,217 | 105,106 | 96.30% | no |
| Method | 4,446 | 4,497 | 98.87% | no |

The build's own confirmation, quoted: `Analyzed bundle 'carddemo-java' with 496 classes` followed by
`All coverage checks have been met.` **Every repeat run over one tree measured these figures to the line** —
23,368 covered of 24,326 in each — which is what a coverage figure should do when the code has not changed.
...
| Report | Line | Branch |
| --- | ---: | ---: |
| Unit only — `target/site/jacoco/` | 94.76% | 88.51% |
| Integration only — `target/site/jacoco-it/` | 74.76% | 59.45% |
| **Merged — `target/site/jacoco-merged/`** | **96.06%** | **89.41%** |
...
The suite behind those figures is the whole estate rather than a sample: the unit tier runs 27,123 test
executions across 451 classes and the integration and end-to-end tier runs 1,666 across 77 classes,
...
**Why the integration tier reads 77 above and 74 in the table below, which is not a discrepancy.** The
sentence counts the classes the tier's inclusion rules select **in this source tree**; the table counts the
**report files** a run wrote. They differ by three because a report file is written per top-level class with
...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| Unit | `target/surefire-reports/` | 451 | 27,123 | 0 | 0 | 0 |
| Integration and end-to-end | `target/failsafe-reports/` | 74 | 1,666 | 0 | 0 | 0 |
| **Whole suite** | both directories | **525** | **28,789** | **0** | **0** | **0** |
...
**Why the integration row's 74 is not the 77 named in the paragraph above, since the two look like the same
number and are not.** The paragraph counts the tier's test *sources* — what the build's include patterns
claim — and the table counts the report files a run *produced*. Three of the 77 are the abstract base classes
...
both 451 and why the equality is what tells `GateVerificationTest` the run was unscoped.
...
method of the same name it counts one. Across this suite that undercounts by **153** — 149 in the unit tier
and 4 in the integration tier. The element count is what the runners' own console totals report, so it is the
...
| Line coverage ≥ 80% | JaCoCo 0.8.15 failing check over `target/jacoco-merged.exec` | `./mvnw -B clean verify` | **PASS (measured)** — 96.06% |
...
| Scan completed | **2026-08-13T23:01:52Z**, the report date carried inside `dependency-check-report.json` itself at `projectInfo.reportDate`, in the recorded run, which is where the dated figures below were read from |
